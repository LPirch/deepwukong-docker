...
def topk_walks(g, nn, num_walks=300, lrp_mode="clip-pos", mode="neuron", negative_transition_strategy="clip-pos", H=None, transforms=None, verbose=False, how_max='none', split_pos_neg=True, init_rel=None):
    ...
    if negative_transition_strategy in ["min_offset", "abs", "clip-pos"]:
...
def plot_top_k_walks(g, top_walks, transforms, rel, mode="neuron", factor=0.3, figname="imgs/topk_walks.eps", color=None, width=12, dataset="MUTAG", 
                    idx=None, compute_rel=True, linewidth=13, node_alpha=1, revert_color=False, node_label_font_size=15):
    ...
    if node_labels is not None:
        pos_labels = pos
        nx.draw_networkx_labels(G, pos_labels, {i: name.split('_')[0] for i, name in enumerate(node_labels)}, font_size=node_label_font_size)
...
def topk_walks_to_node_scores(top_walks, nb_nodes, transforms, rel, score_f="rank-lin", alpha=0.9, floor_factor = 0.75, mode="node", debug=False):
    k = len(top_walks) - 1

    if score_f == "rank-lin":
        score_f = lambda rank,node_times,walk: (k - rank) / k * floor_factor + 1 - floor_factor
    elif score_f == "rank-lin-node-times":
        score_f = lambda rank,node_times,walk: ((k - rank) / k * floor_factor + 1 - floor_factor) * node_times
    elif score_f == "walk-rel":
        score_f = lambda rank,node_times,walk: walk_rel(transforms, rel, walk, mode=mode)
    elif score_f == "rank-lin-walk-rel":
        score_f = lambda rank,node_times,walk: ((k - rank) / k * floor_factor + 1 - floor_factor) * walk_rel(transforms, rel, walk, mode=mode)
    elif score_f == "rank-exp":
        score_f = lambda rank,node_times,walk: (alpha ** rank) * floor_factor + 1 - floor_factor
    elif score_f == "rank-exp-walk-rel":
        score_f = lambda rank,node_times,walk: ((alpha ** rank) * floor_factor + 1 - floor_factor) * walk_rel(transforms, rel, walk, mode=mode)
    elif score_f == "rank-rev-exp":
        score_f = lambda rank,node_times,walk: (1 + alpha ** (k) - alpha ** (k - rank)) * floor_factor + 1 - floor_factor
    elif score_f == "rank-rev-exp-walk-rel":
        score_f = lambda rank,node_times,walk: ((1 + alpha ** (k) - alpha ** (k - rank)) * floor_factor + 1 - floor_factor) * walk_rel(transforms, rel, walk, mode=mode)

    node_score = np.zeros(nb_nodes)

    for i, walk in enumerate(top_walks):
        node_score_tmp = np.zeros(nb_nodes)

        if mode == "node":
            node_walk = walk
        else:
            node_walk = transform_global_neuron_to_node_neuron(walk, transforms)
            node_walk = [node_neuron[0] for node_neuron in node_walk]

        for node in node_walk:
            node_score_tmp[node] += 1

        for node in node_score_tmp.nonzero():
            node_score[node] += score_f(i, node_score_tmp[node], walk)
    
    return node_score

def topk_walks_to_rel_subgraph(top_walks, nb_nodes, transforms, rel, score_f="rank-lin", alpha=0.9, floor_factor = 0.75, subgraph_size=None, debug=False, node_score=None, mode="node"):
    if node_score is None:
        node_score = topk_walks_to_node_scores(top_walks, nb_nodes, transforms, rel, score_f, alpha, floor_factor, mode, debug)

    if subgraph_size is None:
        if debug: print(node_score)
        node_score_non_zero = node_score[node_score.nonzero()]
        score_median = np.sort(node_score_non_zero)[(len(node_score_non_zero)-1)//2: 1+len(node_score_non_zero)//2].mean()
        return np.argwhere(node_score > score_median).flatten().tolist()
    else:
        node_score_sort = node_score.argsort()[::-1]
        return node_score_sort[:subgraph_size].tolist()

def topk_walks_to_node_ordering(top_walks, nb_nodes, transforms, rel, score_f="rank-lin", alpha=0.9, floor_factor = 0.75, subgraph_size=None, debug=False):
    node_score = topk_walks_to_node_scores(top_walks, nb_nodes, transforms, rel, score_f, alpha, floor_factor, debug)
        
    return node_score.argsort()[::-1].tolist()



def topk_node_walks_see_2steps(g, nn, num_walks, lrp_mode="clip-pos", negative_transition_strategy="clip-pos", H=None, transforms=None, verbose=False):
    raise Exception("see 2 steps approx node walk depreciated!")
    if verbose: 
        print("Make sure that you receive two outputs: List of top-k walks, List of time consumption")
        time_list = []
    ...
    if H is None or transforms is None:
        ...
        for i in range(len(transforms)):
            transforms[i] = transforms[i].numpy()
    else:
        if isinstance(H, torch.Tensor): H = H.numpy()
        if isinstance(transforms[0], torch.Tensor):
            for i in range(len(transforms)):
                transforms[i] = transforms[i].numpy()
    
    # Modify transforms in case of negative values
    if negative_transition_strategy == "abs":
        transforms_modified = [np.abs(transform) for transform in transforms]
    elif negative_transition_strategy == "clip-pos":
        transforms_modified = [np.clip(transform, a_min=0, a_max=None) for transform in transforms]
    elif negative_transition_strategy == "none":
        transforms_modified = transforms
    ...
    # Compute the max walk mapping by max-sum
    max_rel_step = (transforms_modified[-1] * init_rel).sum(axis=-1)
    
    for i in range(2, len(transforms_modified)+1):
        max_rel_step = transforms_modified[-i].reshape(list(transforms_modified[-i].shape)+[1]) * max_rel_step.reshape([1,1]+list(max_rel_step.shape))
        mapping = max_rel_step.sum(1).sum(2).argmax(axis=2)

        # print([len(set(mapping[:,i])) for i in range(len(mapping))])

        max_mapping = [mapping] + max_mapping

        for j in range(mapping.shape[0]):
            for k in range(mapping.shape[1]):
                max_rel_step[j,:,k,:,0:mapping[j,k]] = 0
                if mapping[j,k] < max_rel_step.shape[-1]:
                    max_rel_step[j,:,k,:,mapping[j,k]+1:] = 0
                    
        max_rel_step = max_rel_step.sum(axis=-1).sum(axis=-1)

    # last step
    max_rel_step = max_rel_step.sum(1)
    mapping = max_rel_step.argmax(axis=1)
    max_mapping = [mapping] + max_mapping

    for j in range(mapping.shape[0]):
        max_rel_step[j,0:mapping[j]] = 0
        if mapping[j] < max_rel_step.shape[-1]:
            max_rel_step[j,mapping[j]+1:] = 0
    max_rel_step = max_rel_step.sum(-1)
    ...
    for mapping in max_mapping:
        if len(mapping.shape) == 1:
            ...
            max_walk += [mapping[max_walk[-2],max_walk[-1]]] 
    ...
    top_k_max_walks = [max_walk] # to store the top walks
        
    last_max_walk = max_walk
    sub_walk_rel_dict = {}
    for i in range(len(max_walk)):
        sub_walk_rel_dict[tuple(max_walk[:i+1])] = 0
    ...
    for _ in range(num_walks-1):
        ...
        for idx in range(len(last_max_walk)):
            ...
            for feature in range(nb_features):
                sub_walk = np.append(walk, feature)
                if tuple(sub_walk) in sub_walk_rel_dict.keys() or tuple(sub_walk) in top_k_max_walks: continue
                ...
                for mapping in max_mapping[idx:]:
                    # tmp_walk = np.append(tmp_walk, mapping[tmp_walk[-1]]) 
                    if len(tmp_walk) > 1:
                        tmp_walk = np.append(tmp_walk, mapping[tmp_walk[-2], tmp_walk[-1]]) 
                    else:
                        tmp_walk = np.append(tmp_walk, mapping[tmp_walk[-1]]) 

                rel = walk_rel(transforms_modified, init_rel, tmp_walk, mode='node')
